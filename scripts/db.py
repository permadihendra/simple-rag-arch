...
def detect_project(cwd: str | None = None) -> str | None:
    """
    Detect project name from working directory.

    Priority:
    1. Parse 'project:' from rag-context.md frontmatter
    2. Extract from git remote origin URL
    3. Fall back to directory basename

    Returns None if CWD is home directory, simple-rag-arch, or otherwise
    non-project.
    """
    cwd = cwd or os.getcwd()
    cwd_path = Path(cwd).resolve()

    # Skip home and rag system dirs
    home = Path.home()
    rag_dir = Path(__file__).resolve().parent.parent  # simple-rag-arch
    if cwd_path == home or cwd_path == rag_dir or str(rag_dir) in str(cwd_path):
        return None

    # 1. Check rag-context.md frontmatter
    ctx_file = cwd_path / "rag-context.md"
    if ctx_file.exists():
        for line in ctx_file.read_text().split("\n"):
            line = line.strip()
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True, text=True, timeout=3, cwd=str(cwd_path),
        )
        if result.returncode == 0:
            ...
            if repo and repo != "simple-rag-arch":
                return repo
    ...
    # 3. Fall back to directory basename
    name = cwd_path.name
    if name.startswith(".") or name in ("home", "root"):
        return None
    return name


def sync_rag_context(project_name: str, cwd: str | None = None) -> str:
    """
    Generate or refresh rag-context.md from source docs in the project.

    Scans CWD for PLAN.md, AGENTS.md, MEMORY.md, SOUL.md, USER.md.
    If rag-context.md is stale (older than any source), regenerates it.
    Returns the context content (from file or freshly generated).
    """
    cwd = cwd or os.getcwd()
    cwd_path = Path(cwd).resolve()
    ctx_path = cwd_path / "rag-context.md"

    source_files: list[Path] = []
    for fname in ["PLAN.md", "AGENTS.md", "MEMORY.md", "SOUL.md", "USER.md"]:
        fp = cwd_path / fname
        if fp.exists():
            source_files.append(fp)

    # Check freshness — regenerate if stale or missing
    needs_regenerate = True
    if ctx_path.exists() and source_files:
        ctx_mtime = ctx_path.stat().st_mtime
        needs_regenerate = any(fp.stat().st_mtime > ctx_mtime for fp in source_files)

    if needs_regenerate:
        sections = [
            f"# Project: {project_name}",
            "",
            "<!-- Auto-generated by rag. Edit source files to update. -->",
            "",
        ]
        ...
        for fp in source_files:
            ...
            sections.append(f"## From {fp.name}")
            sections.append("")
            sections.append("\n".join(extracted))
            sections.append("")

        if not source_files:
            sections.append("_No source docs found (PLAN.md, AGENTS.md, etc.)_")
            sections.append("")
            sections.append("Add these files to your project root for richer context.")
...
def build_project_context(project_name: str, cwd: str | None = None) -> str:
    """
    Build a formatted project context block for the runtime prompt.

    Combines:
    1. rag-context.md content (auto-generated from source docs)
    2. RAG notes tagged with project:<project_name>
    3. Recent sessions mentioning the project
    """
    cwd = cwd or os.getcwd()
    cwd_path = Path(cwd).resolve()
    parts: list[str] = []

    # 1. rag-context.md content (strip frontmatter)
    ctx_path = cwd_path / "rag-context.md"
    if ctx_path.exists():
        ...
        if ctx_body:
            parts.append(ctx_body)

    # 2. Notes tagged with project:<name>
    tag_pattern = f"project:{project_name}"
    notes = search_notes(tag_pattern, 5)
    if notes:
        parts.append(f"\n📝 Project Notes ({len(notes)}):")
        for n in notes:
            imp = "🟢" if n["importance"] >= 4 else "🟡" if n["importance"] >= 2 else "⚪"
            tags = f" [{n.get('tags', '')}]" if n.get("tags") else ""
            parts.append(f"  {imp} {n['title']}{tags}")

    # 3. Sessions mentioning project
    sessions = search_sessions(project_name, 3)
    if sessions:
        parts.append(f"\n📋 Recent Project Sessions ({len(sessions)}):")
        for s in sessions:
            summary = (s.get("summary") or "(no summary)")[:80]
            parts.append(f"  • Session #{s['id']}: {summary}")
...
def build_runtime_prompt(agent_id: str, max_tokens: int = 6000,
    ...
    If project_name is provided (or auto-detected from CWD), includes
    a ``── Project Context ──`` section with project-specific knowledge.
    """
    import textwrap
